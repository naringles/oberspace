...
const fetchFilesGoogle = async (email,query,drive,socketId)=>{
    ...
    listFiles(oAuth2Client,drive.id,email,query,false,socketId)
    .then (async (response)=>{
      if(response ==='yes'){
        ...
        User.updateOne({"email":email,"drives.id":drive.id}, {"$set":{"drives.$.storageUsage": storageQuota.usage,"drives.$.storageUsageInDrive": storageQuota.usageInDrive,"drives.$.storageUsageInDriveTrash": storageQuota.usageInDriveTrash,"drives.$.storageUsagePct":driveUsgPct}})
        .then(()=>{  
        ...
        .then((driveItems)=>{
         ...
         io.sockets.connected[socketId].emit('googleFiles', {driveId:driveItems.drives[0].id,
        ...
        .catch((err)=>console.log("google fetch file: error in emitting data :",err));
      }
  ...
  const createDefaultFolders= async(auth,email,driveId)=>{
    ...
    drive.files.create({resource: fileMetadata,fields: 'id'}, async (err, file)=> {
      if (err) {
        console.error(err);
      } else {
      ...
      await User.updateOne({"email":email,"drives.id":driveId}, {"$push":{"drives.$.folders": newFolder}});
  ...
  {
    let newFolder = await new Folder({folderId:files[0].id,folderName:files[0].name,folderParentDriveId:driveId});
    await User.updateOne({"email":email,"drives.id":driveId,"drives.folders.folderId": {$ne: files[0].id}},  {"$push":{"drives.$.folders": newFolder}});
  }
  ...
  const listFiles = (auth,driveId,email,query,wholeRefresh,socketId) => {
    return new Promise((resolve, reject) => {
      ...
      getList(drive, "", [],query).then((files) => {
        ...
        if(!wholeRefresh){
          await User.updateOne({"email":email,"drives.id":driveId,"drives.files.fileId": {$ne: file.id}}, {"$push":{"drives.$.files":newFile}});
        }
        else{
          await User.updateOne({"email":email,"drives.id":driveId}, {"$push":{"drives.$.files":newFile}});
          User.findOne({"email":email,"drives.id":driveId},{"drives.$":1})
          .then((driveItems)=>{
           io.sockets.connected[socketId].emit('googleFiles', {driveId:driveItems.drives[0].id,
            storageEmail:driveItems.drives[0].email,
            storageLimit:driveItems.drives[0].storageLimit,
          ...
          .catch((err)=>console.log("google list file error in emitting data :",err));

        }
      }
        });
        if(files.length >0){resolve('yes')}else { resolve('no')}
        });
    });
  };
  ...
  const getList = (drive, pageToken, fileList,query) => {
    return new Promise((resolve) => {
      ...
      drive.files.list(
        ...
        (err, res) => {
          if (err) return console.log("google get list: The API returned an error: " + err);
  ...
  let folderId = await driveIdtoWrite.folders.find(o => o.folderName === file.folderType).folderId;
  ...
  User.updateOne({"email":email,"drives.id":drive.id},{$pull : {"drives.$[].files" : {"fileId":fileId}}})
  .then ((resp)=>  
...
catch(err)
{
  ...
  module.exports ={getProfile,getMeta,fetchFilesGoogle,listFiles,createDefaultFolders,uploadFile,deleteFile};
